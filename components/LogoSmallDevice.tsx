...
const LogoSmallDevice = () => {
  const { toggleOpen, open, setMenuFocused } = useMenuResponsiveStore();
    ...
    <section className="phone:min-w-fit min-w-full mx-0 small:mx-auto hd:mx-0 hd:hidden flex justify-between items-center pt-1">
      <Link href={`/versionalt`} className={`flex items-center justify-center small:gap-x-2 gap-x-1`} onClick={onClickLogo}>
        ...
        <div className="w-full translate-y-1">
          ...
          <h2 className={`font-ysabeau text-brown-logo font-bold tracking-tight small:text-base text-sm`}>Conseillère nutrition spécialiste de la ménopause</h2>
      ...
      {
        open
        ? <X className="small:hidden block text-green-logo cursor-pointer" onClick={() => toggleOpen()} />
        : <Menu className="small:hidden block text-green-logo cursor-pointer" onClick={() => toggleOpen()}/>
